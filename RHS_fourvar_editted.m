...
function dx=RHS_fourvar_editted(~,u,p)
    ...
    dC = p.sigma_C.*F.*(1-C).*(1+p.f_max.*M.*exp(1-M))-p.eta_bar.*M.*C-p.delta_C.*C;
    dF = (1/(p.dx^2)).*Diffusion*F - (p.chi_F/(p.dx^2)).*Chemotaxis + p.sigma_F.*F.*(1-F) - p.delta_F.*F;
